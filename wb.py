...
def insert_db(user_id, user_name):
    try:
        connection = mysql.connector.connect(host='eu-cdbr-west-03.cleardb.net',database='heroku_827e33f01765a32',user='b9aaef9756aadc',password='4817bc17')
        sql_select_Query = "SELECT * FROM IDs"
        sql_query = f"INSERT INTO Ids VALUES ({user_id}, '{user_name}', Null)"
        cursor = connection.cursor()
        cursor.execute(sql_select_Query)
        records = cursor.fetchall()
...
def insert_location_db(user, latitude, longitude):
    ...
    try:
        ...
        if user in ids:
            connection = mysql.connector.connect(host='eu-cdbr-west-03.cleardb.net',database='heroku_827e33f01765a32',user='b9aaef9756aadc',password='4817bc17')
            sql_query = f"UPDATE IDs SET City = '{point_name}' WHERE UserID = {user}"
            cursor = connection.cursor()
            cursor.execute(sql_query)
            connection.commit()
            ...
            return f"{user},\nВи ще не підписані на розсилку, тому не можете змінити локацію!\nБудь ласка, натисніть клавішу 'Щоденний прогноз' щоб підписатись."
    except Error as error:
        return f"Упс, помилка! {error}"
...
def del_location_db(user):
    try:
        connection = mysql.connector.connect(host='eu-cdbr-west-03.cleardb.net',database='heroku_827e33f01765a32',user='b9aaef9756aadc',password='4817bc17')
        sql_query = f"UPDATE IDs SET City = Null WHERE UserID = {user}"
        cursor = connection.cursor()
        cursor.execute(sql_query)
        connection.commit()
...
def send_text(message):
    if message.text == 'Погода сьогодні':
        ...
        bot.send_message(message.chat.id, "Натисніть на клавішу і надайте мені Ваше місцерозташування", reply_markup=keyboard)
