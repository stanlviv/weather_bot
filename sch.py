...
def weather_morning():
    ...
    total = len(records)
    not_delivered = []
    for userid, username, city in records:
        try:
            if city != None:
                weather1 = Weather(city).show_weather()
...
def weather_evening():
    ...
    if wkday == 0 or wkday == 4:
        stiker = quarantine_stickers
    elif wkday == 1 or wkday == 5:
        stiker = podia_stickers
    elif wkday == 2 or wkday == 6:
    ...
    for userid, username, city in records:
        try:
            if city != None:
                forecast1 = Weather(city).show_forecast()
